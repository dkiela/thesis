...
def simBrain(parameters):
	...
	bradb=brainlab.BRAIN(jobname=brainname, fsv = FSV)
...
def injectStim(bradb,parameters,rate,start,end,type):
	...
	stimb.parms['TYPE']='simple_inject'+type+str(int(1000*rand(1)[0])) #random n
	stimb.parms['INJECT']=['COLXY '+type+' E SOMA1_name 1']

	bradb.AddStimInject(stimb)

def runBrain(bradb, d):
