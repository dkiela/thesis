...
def simBrain(parameters):
	...
	# Blue
# XXXXXXX: APPARENTLY THIS MATTERS AS AN ADDITION TO LAYER
#, "_UPPER":100, "_LOWER":67
	n_b = bradb.LAYER({"TYPE":"NB"})
	n_b.AddCellType(ecell,100)
	col.AddLayerType(n_b)

	# Green
	n_g = bradb.LAYER({"TYPE":"NG"})
	n_g.AddCellType(ecell,100)
	col.AddLayerType(n_g)
	
	# Triangle
	n_t = bradb.LAYER({"TYPE":"NT"})
	n_t.AddCellType(ecell,100)
	col.AddLayerType(n_t)
	
	# Square
	n_s = bradb.LAYER({"TYPE":"NS"})
	n_s.AddCellType(ecell,100)
	col.AddLayerType(n_s)
	
	# Inhibitor
	n_i = bradb.LAYER({"TYPE":"INHIBITOR"})
	n_i.AddCellType(icell,100)
