...
while x < 100:
	while y < 100:
		...
		gSp=(f4[x][y]/cGmax)*100

		# only display when correct
		if gSp > bSp and bTp > gTp:
			print "Blue Triangle", bTp
			print "Green Triangle", gTp
			print "Blue Square", bSp
			print "Green Square", gSp
			#avg_diff[x][y]=(abs(bTp-gTp)+abs(bSp-gSp))/2
			avg_diff[x][y]=abs(abs(bTp-gTp)-abs(bSp-gSp))
			#avg_diff[x][y]=(abs(bTp-gTp)/abs(bSp-gSp))*((abs(bTp-gTp)+abs(bSp-gSp))/2)
...
#print avgList[-50:] #display ten best cases
print avgList[:20]
