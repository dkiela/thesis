...
def injectStim(bradb,parameters,rate,end,type):
	...
	stimb.parms['TYPE']='simple_inject'+type
	stimb.parms['INJECT']=['COLXY '+type+' E SOMA1_name 1']

	bradb.AddStimInject(stimb)

def injectBrain(bradb,parameters):
	injectStim(bradb,parameters,parameters['RATE1'],parameters['ENDSIM'],'NB')
	injectStim(bradb,parameters,parameters['RATE1'],parameters['ENDSIM']/2,'NT')
	injectStim(bradb,parameters,parameters['RATE2'],parameters['ENDSIM'],'NG')
	injectStim(bradb,parameters,parameters['RATE2'],parameters['ENDSIM']/2,'NS')

def injectBrain2(bradb,parameters):
	# For Blue Teaching
	s1=bradb.STIMULUS(parms={'MODE': 'VOLTAGE', 'PATTERN':'NOISE'}) 
	s1.parms['AMP_START']='0.3'
	s1.parms['AMP_END']='0.3'
	s1.parms['TIME_START']='0.0'
	s1.parms['TIME_END']=str(parameters['ENDSIM']) # !
	...
	s2.parms['TIME_END']=str(parameters['ENDSIM']) # !
